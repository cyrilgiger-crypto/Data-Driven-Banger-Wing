...
gpr_Clb = GaussianProcessRegressor(
    kernel=kernel,
    normalize_y=True,
    optimizer="fmin_l_bfgs_b",
    n_restarts_optimizer=20
)
gpr_Clb.fit(samples_norm, Clb)
length_scales_Clb = gpr_Clb.kernel_.k2.length_scale

print('-' * 40)
print("Length scales (Clb):")
for name, scale in zip(param_names, length_scales_Clb):
    print(f"  {name}: {scale:.6f}")
...
# 6: c
# 7: delta
param_idx = (4, 2)  # zero-based indices
grid_res = 50
...
ax.set_xlabel(param_labels[param_idx[0]])
ax.set_ylabel(param_labels[param_idx[1]])
ax.set_zlabel("Dutch-Roll Static Stabilty (Clb < 0)")
...
if export_toggle:
    plot_dir = "plot"
    os.makedirs(plot_dir, exist_ok=True)
    filename = f"Clb_3D_surface_{param_labels[param_idx[0]]}_{param_labels[param_idx[1]]}.png"
    filepath = os.path.join(plot_dir, filename)
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
plt.show()
...
cf = ax2.contourf(X_grid, Y_grid, Z_grid, levels=levels, cmap="jet")
cs = ax2.contour(X_grid, Y_grid, Z_grid, levels=levels, colors="k", linewidths=0.3, alpha=0.4)
fig2.colorbar(cf, ax=ax2, shrink=0.8, pad=0.02, label='Dutch-Roll Static Stabilty (Clb < 0)')
...
if export_toggle:
    plot_dir = "plot"
    os.makedirs(plot_dir, exist_ok=True)
    filename = f"Clb_2D_surface_{param_labels[param_idx[0]]}_{param_labels[param_idx[1]]}.png"
    filepath = os.path.join(plot_dir, filename)
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
plt.show()
